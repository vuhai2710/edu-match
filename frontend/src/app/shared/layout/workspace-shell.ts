  ...
  template: `
    @if (activeSegment() === 'admin') {
      <div class="min-h-screen bg-slate-50 flex font-sans">
        ...
        @if (isMobileMenuOpen()) {
          <div class="fixed inset-0 bg-slate-900/40 z-40 md:hidden transition-opacity duration-300" (click)="isMobileMenuOpen.set(false)"></div>
        }

        <!-- Admin Sidebar -->
        <aside [class]="'fixed inset-y-0 left-0 z-50 flex flex-col bg-white border-r-2 border-slate-100 transition-all duration-300 transform md:translate-x-0 md:static md:h-screen md:sticky md:top-0 md:shrink-0 ' + 
                       (isMobileMenuOpen() ? 'translate-x-0 w-64' : '-translate-x-full w-64') + ' ' +
                       (isSidebarCollapsed() ? 'md:w-20' : 'md:w-64')">
          
          <!-- Sidebar Brand Logo -->
          <div class="p-4 border-b-2 border-slate-100 flex items-center justify-between h-[65px] shrink-0">
            <a [routerLink]="dashboardRoute()" class="flex items-center gap-2 group overflow-hidden">
              <div class="w-9 h-9 rounded-xl bg-[#58cc02] flex items-center justify-center shadow-md border-b-3 border-[#4b9b04] shrink-0">
                <svg class="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" />
                </svg>
              </div>
              @if (!isSidebarCollapsed()) {
                <span class="font-display font-bold text-xl text-[#58cc02] transition-opacity duration-200">EduMatch</span>
              }
            </a>
            <!-- Close button for mobile drawer -->
            <button (click)="isMobileMenuOpen.set(false)" class="p-1 rounded-lg hover:bg-slate-100 md:hidden text-slate-400">
          ...
          <div class="p-3 border-t-2 border-slate-100 hidden md:block shrink-0">
            <button (click)="toggleSidebar()" class="w-full flex items-center justify-center p-2.5 rounded-xl hover:bg-slate-50 text-slate-400 hover:text-slate-600 transition-colors">
              @if (isSidebarCollapsed()) {
                <!-- Arrow Right Icon -->
        ...
        <div class="flex-grow flex flex-col min-w-0">
          <!-- Header -->
          <header class="sticky top-0 z-30 w-full bg-white border-b-2 border-slate-100 px-4 py-2.5 shadow-sm h-[65px] flex items-center">
            <div class="w-full flex items-center justify-between gap-2">
              <div class="flex items-center gap-1.5 sm:gap-3 min-w-0">
                <!-- Mobile menu toggle button -->
                <button (click)="isMobileMenuOpen.set(true)" class="p-2 rounded-xl hover:bg-slate-100 md:hidden text-slate-500 shrink-0">
                  <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <!-- Active Page Title -->
                <h1 class="font-display font-bold text-lg md:text-xl text-[#3c3c3c] tracking-wide truncate">{{ activeMenuName() }}</h1>
              </div>
      ...
      <div class="min-h-screen bg-slate-50 flex flex-col font-sans">
        ...
        @if (activeSegment() === 'tutor' || activeSegment() === 'student') {
          <aside [class]="'fixed inset-y-0 left-0 z-50 flex flex-col bg-white border-r-2 border-slate-100 transition-all duration-300 transform md:hidden w-64 ' + 
            ...
            <div class="p-4 border-b-2 border-slate-100 flex items-center justify-between h-[65px] shrink-0">
              ...
              <button (click)="isMobileMenuOpen.set(false)" class="p-1 rounded-lg hover:bg-slate-100 text-slate-400">
                <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
