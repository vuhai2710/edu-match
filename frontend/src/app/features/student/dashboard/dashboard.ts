...
@Component({
  ...
  template: `
    <div class="space-y-6">
      <div
        class="bg-gradient-to-r from-[#58cc02] to-emerald-500 rounded-3xl p-6 md:p-8 flex items-center gap-6 shadow-lg"
      >
        <div class="flex-1 text-white">
          ...
          <p class="mt-1 text-green-100 text-sm md:text-base">
            Theo dõi yêu cầu học, lớp đang học và thanh toán đặt cọc.
          </p>
        </div>
      </div>
      ...
      <div class="grid lg:grid-cols-2 gap-4">
        <section>
          <div class="flex items-center justify-between mb-3">
            ...
            <a
              ...
              class="text-sm font-bold text-duo-blue hover:underline"
              >Xem tất cả</a
            >
          </div>
          <div class="space-y-3">
                ...
                <div class="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p class="text-sm text-slate-500">
                      Gia sư: {{ request.tutorName || 'Đang cập nhật' }}
                    </p>
                  </div>
                  <span
                    [class]="requestClass(request)"
                    class="text-xs font-black rounded-full px-2.5 py-1"
                    >{{ requestLabel(request) }}</span
                  >
                </div>
                <p class="text-xs text-slate-400 mt-2">
                  Cọc dự kiến: {{ money(request.calculatedDepositAmount) }}
                </p>
              </a>
            }
            @if (!requests().length) {
              <div class="tactile-card p-5 text-center text-sm font-bold text-slate-500">
                Chưa có yêu cầu học.
              </div>
            }
          </div>
        ...
        <section>
          <div class="flex items-center justify-between mb-3">
            ...
            <a routerLink="/student/classes" class="text-sm font-bold text-duo-blue hover:underline"
              >Xem tất cả</a
            >
          </div>
          <div class="space-y-3">
              ...
              <a [routerLink]="['/student/classes', item.id]" class="tactile-card p-4 block">
                <div class="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p class="text-sm text-slate-500">
                      Gia sư: {{ item.tutorName || 'Đang cập nhật' }}
                    </p>
                  </div>
                  <span
      ...
      <div class="grid sm:grid-cols-2 gap-4">
        <a
          ...
          class="tactile-card p-5 flex items-center gap-4 hover:shadow-md transition-shadow group"
        >
  ...
  private async loadDashboard(): Promise<void> {
    try {
      const [dashboardResponse, requestsResponse, classesResponse] = await Promise.all([
        ...
        firstValueFrom(
          this.classesApi.getMyClasses(undefined, 1, 5, undefined, 'createdAt', 'desc'),
        ),
      ]);
