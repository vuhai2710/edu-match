...
import { AcademicDegree, ReviewDto, TutorCareerStatus, TutorDetailDto } from '../../../api/generated/client/models';
...
import { formatMoney, academicDegreeLabel, tutorCareerStatusLabel } from '../../../shared/utils/api-ui';
...
@Component({
  ...
  template: `
    @if (tutor(); as t) {
        ...
        <div class="grid lg:grid-cols-3 gap-6">
          <div class="lg:col-span-2 space-y-6">
            <div class="tactile-card p-6">
              <div class="flex items-center gap-4">
                @if (t.avatarUrl) {
                  <img [src]="t.avatarUrl" [alt]="t.fullName" referrerpolicy="no-referrer"
                ...
                <div>
                  <h1 class="font-display text-2xl font-black text-slate-900">{{ t.fullName }}</h1>
                  <p class="text-slate-500 mt-1">{{ t.address?.fullAddress || t.school || 'Chưa cập nhật địa chỉ' }}</p>
                  <div class="flex items-center gap-3 mt-2">
                    <span class="flex items-center gap-1 text-amber-600 font-bold text-sm">★ {{ t.rating ?? 0 }}/5.0</span>
                    <span class="text-slate-400">·</span>
                    <span class="text-sm text-slate-600 font-semibold">{{ t.totalReviews ?? 0 }} đánh giá</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="tactile-card p-6">
              ...
              <p class="text-sm text-slate-600 leading-relaxed">{{ t.profile || 'Gia sư chưa cập nhật phần giới thiệu.' }}</p>
              <div class="grid sm:grid-cols-3 gap-3 mt-4 text-sm">
                ...
                <div class="rounded-xl border-2 border-slate-100 p-3">
                  ...
                  <p class="font-extrabold text-slate-800">{{ getCareerStatus(t.careerStatus) }}</p>
                </div>
                <div class="rounded-xl border-2 border-slate-100 p-3">
                  <p class="text-slate-400 font-bold">Bằng cấp</p>
                  <p class="font-extrabold text-slate-800">{{ getAcademicDegree(t.academicDegree) }}</p>
                </div>
              </div>
            </div>
                ...
                @for (review of reviews(); track review.id) {
                  <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    <div class="flex items-center justify-between mb-2">
                      <p class="font-bold text-sm text-slate-900">{{ review.studentName || 'Học viên' }}</p>
                      <span class="text-xs text-amber-600 font-bold">★ {{ review.rating }}</span>
                    </div>
                    <p class="text-sm text-slate-600">{{ review.comment || 'Không có nhận xét.' }}</p>
                  </div>
                }
                @if (!reviews().length) {
          ...
          <div class="space-y-4">
            <div class="tactile-card p-6 sticky top-20">
              ...
              <a [routerLink]="['/student/chat']" [queryParams]="{ partnerId: t.userId }"
      ...
      <div class="tactile-card p-8 text-center font-bold text-slate-500">Đang tải hồ sơ gia sư...</div>
    } @else {
