    ...
    <div class="space-y-6">
      ...
      <div class="space-y-3">
        ...
        <input type="text"
               ...
               placeholder="Tìm theo mã đơn hàng (orderCode)..."
          ...
          <table class="w-full text-sm">
            <thead class="bg-slate-50 border-b-2 border-slate-100">
              <tr>
                ...
                <th class="px-4 py-3 text-left font-extrabold text-slate-600">Liên quan</th>
              ...
              @for (item of items(); track item.id) {
                <tr class="border-b border-slate-100 hover:bg-slate-50">
                  ...
                  <td class="px-4 py-3 text-xs text-slate-600">
                    <p>LR #{{ item.learningRequestId ?? '—' }}</p>
                    <p>Class #{{ item.classId ?? 'chưa có' }}</p>
                  </td>
